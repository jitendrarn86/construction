...
const getAllMint = async (req, res) => {
  try {
    const users = await Mint.find({is_mint:1}).sort({ _id: -1 });
    res.send(users);
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
};
const getAllHomeMint = async (req, res) => {
  try {
    const users = await Mint.find({is_mint:1}).sort({ _id: -1 }).limit(16);
    res.send(users);
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
};
const getUpcomingMint = async (req, res) => {
  try {
    const users = await Mint.find({is_mint:0}).sort({ _id: -1 }).limit(16);
    res.send(users);
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
};
const getAllUpcomingMint = async (req, res) => {
  try {
    const users = await Mint.find({is_mint:0}).sort({ _id: -1 });
    res.send(users);
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
...
const getMintById = async (req, res) => {
  try {
    const user = await Mint.findOne({ nftid:req.params.id});
    res.send(user);
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const getMintByWalletAddress = async (req, res) => {
  try {
    const user = await Mint.find({ wallet_address:req.params.wallet});
    res.send(user);
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const addBuyRequest = async (req, res) => {
  try {
	  console.log('req.body');
	  console.log(req.body);
	  const newMint = new BuyRequest(req.body);
    await newMint.save();
...
const addTempMint = async (req, res) => {
  try {
	  console.log('req.body');
	  console.log(req.body);
...
const updateTempMint = async (req, res) => {
  try {
	  console.log('updateProfile');
	  console.log(req.body);
	 // const user = await User.findOne({ userwallet: req.body.wallet_address });
	  const user = await Mint.findOne({ nftid:req.body.nftid});;
...
const updateMintNft = async (req, res) => {
  try {
	  console.log('updateProfile');
	  console.log(req.body);
	 // const user = await User.findOne({ userwallet: req.body.wallet_address });
	  const user = await Mint.findOne({ nftid:req.body.nftid});;
		...
		if (user) {
			...
			user.price = req.body.price;
			user.nftid = user.nftid;			
			user.mktid = user.mktid;			
			user.image = user.image;			
			user.contractid = user.contractid;
	...
	res.send({
        ...
        success: true,
		message:'Mint updated Sucessfully'
...
const updateMktMintByNft = async (req, res) => {
  try {
